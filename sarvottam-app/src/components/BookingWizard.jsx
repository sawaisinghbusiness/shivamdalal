...
export default function BookingWizard({ initialService, onClose }) {
  ...
  const rawKey = (params.serviceId || initialService?.id || 'electrician').toLowerCase();
  const serviceKey = rawKey === 'carpainter' ? 'carpenter' : rawKey;
  const serviceConfig = PRICING_CONFIG.serviceEstimates[serviceKey] || PRICING_CONFIG.serviceEstimates.electrician;

  // ── 1. WIZARD STEP STATE ──
  // Step 1: Issue Selection
  // Step 2: Booking Summary (Cart)
  // Step 3: Date & Time
  // Step 4: Address & Location
  // Step 5: Details & Confirm
  const [step, setStep] = useState(1);

  // Step 1 State (Default = NOTHING selected; CTA disabled)
    ...
    try {
      ...
      if (saved) {
        ...
        if (d.cart && d.cart.length > 0) {
          ...
          if (d.step !== undefined) {
            let mapped = d.step;
            if (mapped === 'issue') mapped = 1;
            else if (mapped === 0) mapped = 2;
            else if (mapped === 1) mapped = 3;
            else if (mapped === 2) mapped = 4;
            else if (mapped === 3) mapped = 5;
            setStep(Number(mapped) || 1);
          }
        }
  ...
  const handleProceedFromIssue = () => {
    ...
    setCart([newCartItem]);
    setStep(2); // Step 2: Booking Summary
  ...
  const handleFinalConfirm = () => {
    ...
    const otherServices = cart.slice(1);
    ...
    const newBooking = {
      ...
      servicePhoto: primaryService.photo,
      problem: primaryService.problem,
      brandModel: primaryService.brandModel,
      selectedIssue: primaryService.selectedIssue,
      status: 'PROCESSING',
      dateString: availableDays[selectedDayIdx].fullString,
    ...
    localStorage.removeItem(DRAFT_STORAGE_KEY);
    if (onClose) onClose();
    nav('/booking-success', { state: { booking: newBooking }, replace: true });
  };
  ...
  return (
    <div className="page bw-page">
      ...
      <div className="bw-top-bar">
        ...
        <div className="bw-top-info">
          <h1 className="bw-top-title">
            {step === 1 && `${serviceConfig.name} - Choose Issue`}
            {step === 2 && 'Booking Summary'}
            {step === 3 && 'Choose Date & Time'}
            {step === 4 && 'Service Address'}
            {step === 5 && 'Review & Confirm'}
          </h1>
          <span className="bw-top-step-pill">Step {step} of 5</span>
        </div>
        ...
        {[
          ...
          return (
            <button
              key={st.num}
              type="button"
              className={'bw-st-btn' + (isDone ? ' done' : isActive ? ' active' : '')}
              onClick={() => isDone && goToStep(st.num)}
              disabled={!isDone && !isActive}
            >
              <span className="bw-st-dot">
                {isDone ? <Icon name="check" size={10} /> : st.num}
              </span>
              <span className="bw-st-text">{st.label}</span>
            </button>
          );
        })}
      </div>
      ...
      {step === 1 && (
        <div className="bw-view-scroll" ref={scrollContainerRef} onScroll={handleScroll}>
          {/* Service Hero Banner */}
          <div className="bw-service-hero-card">
            <div className="bw-shc-left">
              <div className="bw-shc-img-wrap">
                <img src={serviceConfig.photo} alt={serviceConfig.name} className="bw-shc-img" />
              </div>
              <div className="bw-shc-info">
                <div className="bw-shc-badge-row">
                  <span className="bw-shc-tag">
                    <Icon name={serviceConfig.badgeIcon} size={13} />
                    <span>{serviceConfig.name}</span>
                  </span>
                </div>
                <h2 className="bw-shc-title">Select Issue / Kaam Chunein</h2>
                <p className="bw-shc-sub">Doorstep inspection &amp; upfront transparent pricing</p>
              </div>
            </div>
            <div className="bw-shc-fee-badge">
              <span className="bw-shc-fee-lbl">Visit Fee</span>
              <strong className="bw-shc-fee-val">₹{PRICING_CONFIG.visitCharge}</strong>
            </div>
          </div>
          ...
          <div className="bw-optional-appliance-box">
            <span className="bw-oab-title">Appliance / Fixture Details (Optional)</span>
      ...
      {step === 2 && (
        <div className="bw-view-scroll" ref={scrollContainerRef} onScroll={handleScroll}>
          <div className="bw-section-head">
            <h3 className="bw-sec-title">Your Service Cart</h3>
            <p className="bw-sec-sub">Single visit charge guarantee for all combined services</p>
          </div>
      ...
      {step === 3 && (
        <div className="bw-view-scroll" ref={scrollContainerRef} onScroll={handleScroll}>
          ...
          {/* Horizontal Date Chips (Today + 4 days) */}
      ...
      {step === 4 && (
        <div className="bw-view-scroll" ref={scrollContainerRef} onScroll={handleScroll}>
          ...
          <div className="bw-geo-trigger-card">
            <div className="bw-gtc-left">
              <span className="bw-gtc-icon">
                <Icon name="compass" size={18} />
              </span>
              <div>
                <strong className="bw-gtc-title">Auto-Detect Current Location</strong>
                <p className="bw-gtc-sub">Use GPS for instant doorstep pin</p>
              </div>
            </div>
            <button
              type="button"
              className="bw-gtc-btn"
              disabled={isLocating}
          ...
          {/* Visual Address Confirmation Card */}
          <div className="bw-addr-display-card">
            <div className="bw-adc-header">
              <div className="bw-adc-left">
                <Icon name="pin" size={18} />
                <span className="bw-adc-badge">Verified Pin</span>
              </div>
              <span className="bw-adc-area-pill">{addressArea}</span>
            </div>

            <textarea
              className="bw-adc-input"
              rows={3}
              value={addressDetail}
              onChange={(e) => setAddressDetail(e.target.value)}
              placeholder="House/Plot No., Street/Road, Landmark, Barmer…"
            />
            {addressDetail.trim().length < 10 && (
              <small className="bw-input-warn">Please enter complete address details (min 10 characters)</small>
            )}
          </div>
        ...
        <div className="bw-view-scroll" ref={scrollContainerRef} onScroll={handleScroll}>
          ...
          <div className="bw-card-block bw-pricing-card">
            ...
            <div className="bw-pc-rows">
              ...
              <div className="bw-pc-divider" />

              <div className="bw-pc-row total">
                <strong>Estimated Total Payable</strong>
                <strong>₹{cartPricingSummary.totalMin} – ₹{cartPricingSummary.totalMax}</strong>
              </div>
            </div>

            <div className="bw-pc-notice">
              <Icon name="info" size={14} />
              <span>Visiting charge is ₹{cartPricingSummary.visitCharge}. Final labor payable after service satisfaction via Cash or UPI.</span>
            </div>
          </div>

          {/* Platform Trust Badges */}
          <div className="bw-trust-pills-row">
            <div className="bw-tp-item">
